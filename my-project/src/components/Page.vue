<template>
    <div>
        <!--视频-->
        <video :src="current.path" controls="controls"
              ></video>
        <!--视频结束-->

        <h1 v-for="v in title">{{v.title}}</h1>
        <ul id="list">
...
<script>
    export default {
        ...
        mounted(){
            ...
            this.axios.get('http://laravelvideo.com/api/VideoTitle/'+lessonId).then((response)=>{
                this.title = response.data.data;
            })

            this.axios.get('http://laravelvideo.com/api/videos/'+lessonId).then((response)=>{
